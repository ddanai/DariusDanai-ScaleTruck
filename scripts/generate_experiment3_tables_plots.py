#!/usr/bin/env python3
"""Generate publication-ready Experiment 3 tables and plots from its summary."""
...
BASE = ROOT / "results" / "latency" / "experiment3" / "comparison"
SUMMARY = BASE / "experiment3-summary.json"
PLOTS = BASE / "plots"
...
def write_tables(summary):
    ...
    with (BASE / "experiment3-latency-comparison.csv").open("w", newline="") as handle:
    ...
    with (BASE / "experiment3-timing-comparison.csv").open("w", newline="") as handle:
    ...
    (BASE / "experiment3-comparison-table.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8")
...
def plot_latency(summary):
    ...
    save_figure(fig, "experiment3-latency-differences")
...
def plot_timing(summary):
    ...
    for ax in axes:
        ...
        ax.legend()
    fig.suptitle("Topic timing across 10 matched runs per system")
    save_figure(fig, "experiment3-topic-timing")
...
def plot_resources(summary):
    ...
    for ax in axes:
        ax.set_xticks(x, labels)
        ax.grid(axis="y", alpha=0.25)
    fig.suptitle("Session-level resources (mean ± run SD)\nNot a middleware-only comparison")
    save_figure(fig, "experiment3-session-resources")
